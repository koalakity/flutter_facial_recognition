...
public class FaceHelper {
    ...
    public List<FacePreviewInfo> onPreviewFrame(byte[] nv21, Size previewSize) {
        if (faceListener != null) {
            if (ftEngine != null) {
                faceInfoList.clear();
                long ftStartTime = System.currentTimeMillis();
                int code = ftEngine.detectFaces(nv21, previewSize.getWidth(), previewSize.getHeight(), FaceEngine.CP_PAF_NV21, faceInfoList);
                if (code != ErrorInfo.MOK) {
                    Log.i("onPreviewFrame","ft failed,code is " + code);
